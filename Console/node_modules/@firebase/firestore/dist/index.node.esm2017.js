    ...
    applyChanges(transaction) {
        ...
        this.changes.forEach((key, documentChange) => {
            ...
            if (documentChange.maybeDocument) {
                const doc = toDbRemoteDocument(this.documentCache.serializer, documentChange.maybeDocument, this.getReadTime(key));
                ...
                if (this.trackRemovals) {
                    ...
                    const deletedDoc = toDbRemoteDocument(this.documentCache.serializer, new NoDocument(key, SnapshotVersion.min()), this.getReadTime(key));
...
class SimpleDb {
    ...
    static delete(name) {
        logDebug(LOG_TAG$1, 'Removing database:', name);
    ...
    async ensureDb(action) {
        if (!this.db) {
            logDebug(LOG_TAG$1, 'Opening database:', this.name);
            this.db = await new Promise((resolve, reject) => {
                ...
                request.onupgradeneeded = (event) => {
                    logDebug(LOG_TAG$1, 'Database "' + this.name + '" requires upgrade from version:', event.oldVersion);
                    const db = event.target.result;
                    this.schemaConverter
                        .createOrUpgrade(db, request.transaction, event.oldVersion, this.version)
                        .next(() => {
                        logDebug(LOG_TAG$1, 'Database upgrade to version ' + this.version + ' complete');
                    });
                };
            });
        }
    ...
    async runTransaction(action, mode, objectStores, transactionFn) {
        ...
        while (true) {
            ...
            try {
                ...
                logDebug(LOG_TAG$1, 'Transaction failed with error:', error.message, 'Retrying:', retryable);
                this.close();
                if (!retryable) {
...
class SimpleDbTransaction {
    ...
    abort(error) {
        ...
        if (!this.aborted) {
            logDebug(LOG_TAG$1, 'Aborting transaction:', error ? error.message : 'Client-initiated abort');
            this.aborted = true;
...
class SimpleDbStore {
    ...
    put(keyOrValue, value) {
        let request;
        if (value !== undefined) {
            logDebug(LOG_TAG$1, 'PUT', this.store.name, keyOrValue, value);
            request = this.store.put(value, keyOrValue);
        }
        else {
            logDebug(LOG_TAG$1, 'PUT', this.store.name, '<auto-key>', keyOrValue);
     ...
     */
    add(value) {
        logDebug(LOG_TAG$1, 'ADD', this.store.name, value, value);
    ...
    get(key) {
        ...
        return wrapRequest(request).next(result => {
            ...
            logDebug(LOG_TAG$1, 'GET', this.store.name, key, result);
            return result;
        });
    }
    delete(key) {
        logDebug(LOG_TAG$1, 'DELETE', this.store.name, key);
     ...
     */
    count() {
        logDebug(LOG_TAG$1, 'COUNT', this.store.name);
    ...
    deleteAll(indexOrRange, range) {
        logDebug(LOG_TAG$1, 'DELETE ALL', this.store.name);
...
function bundlesStore(txn) {
    return IndexedDbPersistence.getStore(txn, DbBundle.store);
}
/**
 * Helper to get a typed SimpleDbStore for the namedQueries object store.
 */
function namedQueriesStore(txn) {
    return IndexedDbPersistence.getStore(txn, DbNamedQuery.store);
}
...
// change.
async function handleUserChange(localStore, user) {
 ...
 */
async function notifyLocalViewChanges(localStore, viewChanges) {
    const localStoreImpl = debugCast(localStore);
    try {
...
async function releaseTarget(localStore, targetId, keepPersistedTargetData) {
    const localStoreImpl = debugCast(localStore);
    const targetData = localStoreImpl.targetDataByTarget.get(targetId);
 ...
 * limitations under the License.
 */
const LOG_TAG$6 = 'PersistentStream';
...
class PersistentStream {
    ...
    handleStreamClose(error) {
        logDebug(LOG_TAG$6, `close with error: ${error}`);
        this.stream = null;
        ...
        return (fn) => {
            this.queue.enqueueAndForget(() => {
                if (this.closeCount === startCloseCount) {
                    ...
                    logDebug(LOG_TAG$6, 'stream callback skipped by getCloseGuardedDispatcher.');
                    return Promise.resolve();
                }
            });
        };
 ...
 * limitations under the License.
 */
const LOG_TAG$7 = 'OnlineStateTracker';
        ...
        this.connectivityMonitor.addCallback((_) => {
            asyncQueue.enqueueAndForget(async () => {
                ...
                if (canUseNetwork(this)) {
                    logDebug(LOG_TAG$8, 'Restarting streams for network reachability change.');
...
async function remoteStoreShutdown(remoteStore) {
    const remoteStoreImpl = debugCast(remoteStore);
    logDebug(LOG_TAG$8, 'RemoteStore shutting down.');
...
async function onWatchStreamChange(remoteStoreImpl, watchChange, snapshotVersion) {
    ...
    if (watchChange instanceof WatchTargetChange &&
        ...
        try {
            ...
            logDebug(LOG_TAG$8, 'Failed to remove targets %s: %s ', watchChange.targetIds.join(','), e);
    ...
    if (!snapshotVersion.isEqual(SnapshotVersion.min())) {
        try {
            const lastRemoteSnapshotVersion = await getLastRemoteSnapshotVersion(remoteStoreImpl.localStore);
            ...
            logDebug(LOG_TAG$8, 'Failed to raise snapshot:', e);
            await disableNetworkUntilRecovery(remoteStoreImpl, e);
        }
    }
}
...
async function disableNetworkUntilRecovery(remoteStoreImpl, e, op) {
    if (isIndexedDbTransactionError(e)) {
        ...
        remoteStoreImpl.asyncQueue.enqueueRetryable(async () => {
            logDebug(LOG_TAG$8, 'Retrying IndexedDB access');
            await op();
...
async function fillWritePipeline(remoteStore) {
    ...
    while (canAddToWritePipeline(remoteStoreImpl)) {
        try {
            const batch = await nextMutationBatch(remoteStoreImpl.localStore, lastBatchIdRetrieved);
            if (batch === null) {
...
async function remoteStoreHandleCredentialChange(remoteStore, user) {
    ...
    logDebug(LOG_TAG$8, 'RemoteStore received new credentials');
    ...
    if (!remoteStoreImpl.writeStream) {
        ...
        remoteStoreImpl.onNetworkStatusChange.push(async (enabled) => {
            if (enabled) {
                ...
                if (remoteStoreImpl.writePipeline.length > 0) {
                    logDebug(LOG_TAG$8, `Stopping write stream with ${remoteStoreImpl.writePipeline.length} pending writes`);
...
class AsyncQueue {
    ...
    async retryNextOp() {
        ...
        try {
            ...
            if (isIndexedDbTransactionError(e)) {
                logDebug(LOG_TAG$9, 'Operation failed with retryable error: ' + e);
            }
            else {
...
async function syncEngineListen(syncEngine, query) {
    ...
    if (queryView) {
        ...
        const targetData = await allocateTarget(syncEngineImpl.localStore, queryToTarget(query));
...
async function initializeViewAndComputeSnapshot(syncEngineImpl, query, targetId, current) {
    ...
    const queryResult = await executeQuery(syncEngineImpl.localStore, query, 
...
async function syncEngineUnlisten(syncEngine, query) {
    ...
    if (syncEngineImpl.isPrimaryClient) {
        ...
        if (!targetRemainsActive) {
            await releaseTarget(syncEngineImpl.localStore, queryView.targetId, 
 ...
 */
async function applyRemoteEvent(syncEngine, remoteEvent) {
    const syncEngineImpl = debugCast(syncEngine);
    try {
        const changes = await applyRemoteEventToLocalCache(syncEngineImpl.localStore, remoteEvent);
 ...
 */
async function rejectListen(syncEngine, targetId, err) {
    const syncEngineImpl = debugCast(syncEngine);
    // PORTING NOTE: Multi-tab only.
    ...
    if (limboKey) {
        ...
        await releaseTarget(syncEngineImpl.localStore, targetId, 
        /* keepPersistedTargetData */ false)
            .then(() => removeAndCleanupTarget(syncEngineImpl, targetId, err))
            .catch(ignoreIfPrimaryLeaseLoss);
    }
}
async function applySuccessfulWrite(syncEngine, mutationBatchResult) {
    const syncEngineImpl = debugCast(syncEngine);
    const batchId = mutationBatchResult.batch.batchId;
    try {
        const changes = await acknowledgeBatch(syncEngineImpl.localStore, mutationBatchResult);
...
async function rejectFailedWrite(syncEngine, batchId, error) {
    const syncEngineImpl = debugCast(syncEngine);
    try {
 ...
 * are acknowledged .
 */
async function registerPendingWritesCallback(syncEngine, callback) {
    const syncEngineImpl = debugCast(syncEngine);
    if (!canUseNetwork(syncEngineImpl.remoteStore)) {
        logDebug(LOG_TAG$a, 'The network is disabled. The task returned by ' +
            "'awaitPendingWrites()' will not complete until the network is enabled.");
    }
    try {
        const highestBatchId = await getHighestUnacknowledgedBatchId(syncEngineImpl.localStore);
...
function updateTrackedLimbos(syncEngineImpl, targetId, limboChanges) {
    for (const limboChange of limboChanges) {
        if (limboChange instanceof AddedLimboDocument) {
            ...
            logDebug(LOG_TAG$a, 'Document no longer in limbo: ' + limboChange.key);
...
function trackLimboChange(syncEngineImpl, limboChange) {
    ...
    if (!syncEngineImpl.activeLimboTargetsByKey.get(key)) {
        logDebug(LOG_TAG$a, 'New document in limbo: ' + key);
...
async function emitNewSnapsAndNotifyLocalStore(syncEngine, changes, remoteEvent) {
    const syncEngineImpl = debugCast(syncEngine);
    const newSnaps = [];
...
async function applyDocChanges(syncEngineImpl, queryView, changes, remoteEvent) {
    ...
    if (viewDocChanges.needsRefill) {
        ...
        viewDocChanges = await executeQuery(syncEngineImpl.localStore, queryView.query, 
...
async function syncEngineHandleCredentialChange(syncEngine, user) {
    ...
    if (userChanged) {
        logDebug(LOG_TAG$a, 'User change. New user:', user.toKey());
        ...
        await emitNewSnapsAndNotifyLocalStore(syncEngineImpl, result.affectedDocuments);
    }
}
...
async function synchronizeViewAndComputeSnapshot(syncEngine, queryView) {
    const syncEngineImpl = debugCast(syncEngine);
    const queryResult = await executeQuery(syncEngineImpl.localStore, queryView.query, 
    /* usePreviousResults= */ true);
 ...
 */
// PORTING NOTE: Multi-Tab only.
async function synchronizeWithChangedDocuments(syncEngine) {
    const syncEngineImpl = debugCast(syncEngine);
    return getNewDocumentChanges(syncEngineImpl.localStore).then(changes => emitNewSnapsAndNotifyLocalStore(syncEngineImpl, changes));
}
...
async function applyBatchState(syncEngine, batchId, batchState, error) {
    ...
    if (documents === null) {
        ...
        // as applicable.
        logDebug(LOG_TAG$a, 'Cannot apply mutation batch with id: ' + batchId);
        return;
    }
...
async function synchronizeQueryViewsAndRaiseSnapshots(syncEngine, targets, transitionToPrimary) {
    ...
    for (const targetId of targets) {
        ...
        if (queries && queries.length !== 0) {
            ...
            targetData = await allocateTarget(syncEngineImpl.localStore, queryToTarget(queries[0]));
            for (const query of queries) {
                const queryView = syncEngineImpl.queryViewsByQuery.get(query);
...
async function applyTargetState(syncEngine, targetId, state, error) {
    ...
    if (syncEngineImpl._isPrimaryClient) {
        ...
        logDebug(LOG_TAG$a, 'Ignoring unexpected query state notification.');
        return;
    }
    if (syncEngineImpl.queriesByTarget.has(targetId)) {
        switch (state) {
            ...
            case 'not-current': {
                ...
                await emitNewSnapsAndNotifyLocalStore(syncEngineImpl, changes, synthesizedRemoteEvent);
                break;
            }
            case 'rejected': {
                await releaseTarget(syncEngineImpl.localStore, targetId, 
...
async function applyActiveTargetsChange(syncEngine, added, removed) {
    ...
    for (const targetId of added) {
        if (syncEngineImpl.queriesByTarget.has(targetId)) {
            ...
            logDebug(LOG_TAG$a, 'Adding an already active target ' + targetId);
            continue;
        }
        const target = await getCachedTarget(syncEngineImpl.localStore, targetId);
 ...
 * limitations under the License.
 */
const LOG_TAG$b = 'SharedClientState';
/**
...
class MutationMetadata {
    ...
    static fromWebStorageEntry(user, batchId, value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$b, `Failed to parse mutation state for ID '${batchId}': ${value}`);
            return null;
        }
    }
    toWebStorageJSON() {
...
class QueryTargetMetadata {
    ...
    static fromWebStorageEntry(targetId, value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$b, `Failed to parse target state for ID '${targetId}': ${value}`);
            return null;
        }
    }
    toWebStorageJSON() {
...
class RemoteClientState {
    ...
    static fromWebStorageEntry(clientId, value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$b, `Failed to parse client data for instance '${clientId}': ${value}`);
            return null;
        }
    }
}
...
class SharedOnlineState {
    ...
    static fromWebStorageEntry(value) {
        ...
        if (validData) {
            ...
            logError(LOG_TAG$b, `Failed to parse online state: ${value}`);
            return null;
        }
    }
}
...
class WebStorageSharedClientState {
    ...
    getItem(key) {
        ...
        logDebug(LOG_TAG$b, 'READ', key, value);
        return value;
    }
    setItem(key, value) {
        logDebug(LOG_TAG$b, 'SET', key, value);
        this.storage.setItem(key, value);
    }
    removeItem(key) {
        logDebug(LOG_TAG$b, 'REMOVE', key);
    ...
    handleWebStorageEvent(event) {
        ...
        if (storageEvent.storageArea === this.storage) {
            logDebug(LOG_TAG$b, 'EVENT', storageEvent.key, storageEvent.newValue);
    ...
    async handleMutationBatchEvent(mutationBatch) {
        if (mutationBatch.user.uid !== this.currentUser.uid) {
            logDebug(LOG_TAG$b, `Ignoring mutation for non-active user ${mutationBatch.user.uid}`);
            return;
        }
...
function fromWebStorageSequenceNumber(seqString) {
    ...
    if (seqString != null) {
        try {
            ...
            logError(LOG_TAG$b, 'Failed to read sequence number from WebStorage', e);
        }
    }
    ...
    applyChanges(transaction) {
        ...
        this.changes.forEach((key, doc) => {
            if (doc && doc.maybeDocument) {
                promises.push(this.documentCache.addEntry(transaction, doc.maybeDocument, this.getReadTime(key)));
            }
            else {
...
const X_GOOG_API_CLIENT_VALUE = `gl-node/${process.versions.node} fire/${version} grpc/${version$2}`;
...
class MemoryOfflineComponentProvider {
    ...
    createLocalStore(cfg) {
        return newLocalStore(this.persistence, new IndexFreeQueryEngine(), cfg.initialUser, this.serializer);
    }
    ...
    async initialize(cfg) {
        ...
        if (this.sharedClientState instanceof WebStorageSharedClientState) {
            this.sharedClientState.syncEngine = {
                ...
                synchronizeWithChangedDocuments: synchronizeWithChangedDocuments.bind(null, syncEngine)
            };
...
class OnlineComponentProvider {
    async initialize(offlineComponentProvider, cfg) {
        ...
        this.sharedClientState.onlineStateHandler = onlineState => applyOnlineStateChange(this.syncEngine, onlineState, 1 /* SharedClientState */);
    ...
    createRemoteStore(cfg) {
        return newRemoteStore(this.localStore, this.datastore, cfg.asyncQueue, onlineState => applyOnlineStateChange(this.syncEngine, onlineState, 0 /* RemoteStore */), newConnectivityMonitor());
    }
...
async function setOfflineComponentProvider(client, offlineComponentProvider) {
    ...
    client.setCredentialChangeListener(user => client.asyncQueue.enqueueRetryable(async () => {
...
async function executeQueryFromCache(localStore, query, result) {
    try {
        const queryResult = await executeQuery(localStore, query, 
        /* usePreviousResults= */ true);
        const view = new View(query, queryResult.remoteKeys);
        const viewDocChanges = view.computeDocChanges(queryResult.documents);
        const viewChange = view.applyChanges(viewDocChanges, 
 ...
 */
// settings() defaults:
const DEFAULT_HOST$1 = 'firestore.googleapis.com';
...
function getDoc(reference) {
    ...
    firestore._queue.enqueueAndForget(async () => {
        ...
        await readDocumentViaSnapshotListener(eventManager, firestore._queue, reference._key, { source: 'default' }, deferred);
    });
    return deferred.promise.then(snapshot => convertToDocSnapshot(firestore, reference, snapshot));
}
...
function getDocFromCache(reference) {
    ...
    return deferred.promise.then(doc => new DocumentSnapshot$1(firestore, userDataWriter, reference._key, doc, new SnapshotMetadata(doc instanceof Document ? doc.hasLocalMutations : false, 
...
function getDocFromServer(reference) {
    ...
    firestore._queue.enqueueAndForget(async () => {
        const eventManager = await getEventManager(client);
        await readDocumentViaSnapshotListener(eventManager, firestore._queue, reference._key, { source: 'server' }, deferred);
    });
    return deferred.promise.then(snapshot => convertToDocSnapshot(firestore, reference, snapshot));
}
/**
...
function getDocs(query) {
    ...
    firestore._queue.enqueueAndForget(async () => {
        const eventManager = await getEventManager(client);
        await executeQueryViaSnapshotListener(eventManager, firestore._queue, query._query, { source: 'default' }, deferred);
    });
    return deferred.promise.then(snapshot => new QuerySnapshot(firestore, userDataWriter, query, snapshot));
}
/**
...
function getDocsFromServer(query) {
    ...
    firestore._queue.enqueueAndForget(async () => {
        const eventManager = await getEventManager(client);
        await executeQueryViaSnapshotListener(eventManager, firestore._queue, query._query, { source: 'server' }, deferred);
    });
    return deferred.promise.then(snapshot => new QuerySnapshot(firestore, userDataWriter, query, snapshot));
}
...
function updateDoc(reference, fieldOrUpdateData, value, ...moreFieldsAndValues) {
    ...
    const mutations = parsed.toMutations(reference._key, Precondition.exists(true));
    return executeWrite(firestore, mutations);
}
/**
...
function onSnapshot(reference, ...args) {
    ...
    if (reference instanceof DocumentReference) {
        ...
        observer = {
            next: snapshot => {
                if (args[currArg]) {
                    args[currArg](new QuerySnapshot(firestore, userDataWriter, reference, snapshot));
                }
            },
            error: args[currArg + 1],
            complete: args[currArg + 2]
    ...
    const listener = new QueryListener(internalQuery, wrappedObserver, internalOptions);
    firestore._queue.enqueueAndForget(async () => {
        const eventManager = await getEventManager(client);
        return eventManagerListen(eventManager, listener);
    });
 ...
 */
class Firestore extends Compat {
    constructor(databaseIdOrApp, delegate, _persistenceProvider) {
        super(delegate);
...
function configureFirestore(firestore) {
    const settings = firestore._freezeSettings();
    const databaseInfo = makeDatabaseInfo(firestore._databaseId, firestore._persistenceKey, settings);
 ...
 * A reference to a transaction.
 */
class Transaction$3 extends Compat {
    constructor(_firestore, delegate) {
        super(delegate);
        this._firestore = _firestore;
    }
    get(documentRef) {
        ...
        return this._delegate
            .get(ref)
            .then(result => new DocumentSnapshot$2(this._firestore, result));
    }
    set(documentRef, data, options) {
        const ref = castReference(documentRef);
        if (options) {
...
class WriteBatch$1 extends Compat {
    set(documentRef, data, options) {
        const ref = castReference(documentRef);
        if (options) {
 ...
 */
class DocumentReference$1 extends Compat {
    constructor(firestore, delegate) {
        super(delegate);
        this.firestore = firestore;
...
class DocumentSnapshot$2 extends Compat {
    constructor(_firestore, delegate) {
        super(delegate);
        this._firestore = _firestore;
    }
...
class Query$1 extends Compat {
    ...
    get(options) {
        ...
        return query.then(result => new QuerySnapshot$1(this.firestore, result));
    }
    onSnapshot(...args) {
        const options = extractSnapshotOptions(args);
        const observer = wrapObserver(args, snap => new QuerySnapshot$1(this.firestore, snap));
        return onSnapshot(this._delegate, options, observer);
    }
    withConverter(converter) {
...
class DocumentChange extends Compat {
    constructor(_firestore, delegate) {
        super(delegate);
        this._firestore = _firestore;
    }
...
class QuerySnapshot$1 extends Compat {
    constructor(_firestore, delegate) {
        super(delegate);
        this._firestore = _firestore;
    }
 ...
 */
class FieldPath$2 extends Compat {
    /**
     * Creates a FieldPath from the provided field names. If more than one field
     * name is provided, the path will point to a nested field in a document.
     *
     ...
     */
    constructor(...fieldNames) {
        super(new FieldPath$1(...fieldNames));
    }
    static documentId() {
        /**
 ...
 * limitations under the License.
 */
class FieldValue$1 extends Compat {
    static serverTimestamp() {
        const delegate = serverTimestamp$1();
...
function registerFirestore(instance) {
    configureForFirebase(instance, (app, auth) => new Firestore(app, new FirebaseFirestore$1(app, auth), new IndexedDbPersistenceProvider()));
